  ...
  return (
    <div className="flex flex-col space-y-1 my-10 relative z-[100]">
      ...
      {socials.map((link: any) => {
        if (link.action === "modal" && link.id === "wechat") {
          return (
            <button
              key={link.id}
      ...
      <WechatModal 
        ...
        wechatId={socials.find(s => s.id === "wechat")?.wechatId || "your_wechat_id"}
      />
    </div>
  );
};
  ...
  return (
    <div className="flex space-x-2">
      ...
      <div className="flex text-sm flex-col">
        <p className="font-bold text-primary">刘一民</p>
        <p className="font-light text-secondary">学生 AI工程师</p>
      </div>
    </div>
  );
};
